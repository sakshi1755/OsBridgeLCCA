...
const EconomicCost100YearsRadialChart = () => {
  const svgRef = useRef();

  useEffect(() => {
    // Data from the Economic Cost 100 years graph
    const economicCostData = {
      initialStage: 90, // outermost ring
      useStage: 75,     // second ring
      endOfLifeStage: 63, // third ring
      beyondLifeStage: 52 // innermost ring
    };

    // Calculate percentages (already provided from image)
    const percentage = [
      economicCostData.initialStage,
      economicCostData.useStage,
      economicCostData.endOfLifeStage,
      economicCostData.beyondLifeStage
    ];

    // Colors matching the original economic theme
   const colors = [ '#273B5C', '#961818', '#5A003B','#36454F'];
    ...
    // Create tooltip
    const tooltip = d3.select("body").select(".economic-100-tooltip");
    let tooltipDiv;
    if (tooltip.empty()) {
      tooltipDiv = d3.select("body").append("div")
        .attr("class", "economic-100-tooltip")
            ...
            <div style="text-align:center; font-family:sans-serif;">
              ...
              <span style="color: #638B48; font-size: 13.5px;">
                ...
                <span style="font-weight: 600;">
                  ${(hoveredData.value * 100).toFixed(0)}%
                </span>
              </span>
            </div>
          `)
    ...
    <div className="w-full h-full">
      <div style={{ 
        ...
        <h2 style={{ 
          ...
          <span style={{ color: '#638B48' }}>Economic cost</span> distribution across various stages for bridges for 100 years
        </h2>
...
export default EconomicCost100YearsRadialChart;