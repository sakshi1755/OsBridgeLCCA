...
class CustomTabWidget(QWidget):
    ...
    def activate_tab(self, index):
        ...
        if 0 <= index < self.tab_counter:
            self.setCurrentIndex(index)
        else:
            raise ValueError(f"Invalid tab index: {index}. Must be between 0 and {self.count() - 1}.")
    ...
    def close_tab(self, index):
        ...
        for tab in self.parent.active_tab_widgets.keys():
